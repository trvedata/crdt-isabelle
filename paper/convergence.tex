...
Nodes exchange updates with other nodes asynchronously when a network connection is available.  
...
Convergence implies that whenever two nodes have received the same set of operations, in \emph{any} order, their states must be equivalent---in other words, all operations must be commutative.
...
Our intuition for convergence is that nodes end up in the same state if they have applied the same set of operations.
In order to reach a precise definition of convergence, we must define what a state is and how it can be changed.
For generality, we use an abstract type variable $\isacharprime\isa{state}$ for the state of a node, and we do not assume anything about its properties.
