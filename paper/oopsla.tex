...
In this paper we adopted a ``foundational'' approach to proving the correctness of a class of strong eventual consistency algorithms: the Conflict-free Replicated Datatypes (CRDTs).
In our work, we avoided axioms related to any individual algorithm, and instead build up from a formal, realistic model of a computer network which may delay, drop or reorder messages sent between computers.
We then complemented this model with an abstract specification of strong eventual consistency.
These two components form a framework, or generic substrate, upon which one can prove convergence for concrete CRDT implementations.
As evidence of this, we used our framework to show that three operation-based CRDT algorithms---the Replicated Growable Array, the Observed-Remove Set, and a Counter---do indeed provide the SEC property under all possible valid behaviours of our network model.
...
Coupling our abstract convergence theorem with a CRDT-specific technical lemma showing that the concurrent delivered messages of a prefix of a local history of a node commute, it is easy to prove convergence---in fact, the same single line of Isabelle proof is used to obtain the convergence theorem as a corollary, in all of our three concrete CRDT implementations.

As this recipe described above demonstrates, we have not only isolated a series of reusable lemmas and models of networks, but also identified a fixed proof strategy that algorithm designers can use to obtain a convergence theorem for their operation-based CRDT.
We make this precise with the $\isa{strong}{\isacharunderscore}\isa{eventual}{\isacharunderscore}\isa{consistency}$ locale, which provides sufficient conditions in order to obtain a convergence theorem for a replicated type.
...
By line count, over half (around 1000 lines) of our development---the network model, convergence proof, and so on---is CRDT-agnostic and reusable in future proofs.
...
Operational Transformation algorithms which support the $\mathit{TP}_1$ alone are already widely used in systems such as Google Docs, however these require all clients communicate changes via a single central server.
Similarly, state-based CRDTs are used in systems such as Riak.
Both of these approaches are limiting.
In the case of Operational Transformation, the requirement for a central server prevents offline editing and direct collaboration via local wireless network connections.
In the case of state-based CRDTs, algorithms to support complex data structures such as ordered lists are yet to be found, and in any case are likely impractical if small changes are regularly made to lists containing many items due to the requirement to broadcast the state.
Consequently, our approach provides the groundwork for a new generation of applications which use truly distributed strong eventual consistency alogrithms, including robust, collaborative applications working on complex data structures in a peer-to-peer setting, something which is likely to become increasingly important in a world where mobile devices are becoming increasingly prevalent.
Nevertheless, we speculate that our framework is also amenable to the inclusion of strong eventual consistency algorithms based on Operational Transformation as well as state-based CRDTs, and not limited to operation-based algorithms, which we have focussed on in this work.
Further, we also speculate that our framework could be used to demonstrate the equivalence of classes of strong eventual consistency algorithms.
...
\begin{acks}
    ...
    The authors wish to acknowledge the support of the~\grantsponsor{GS501100000266}{EPSRC}{http://dx.doi.org/10.13039/501100000266} ``REMS: Rigorous Engineering for Mainstream Systems'' programme grant (EP/K008528),
     and the~\grantsponsor{GS501100000266}{EPSRC}{http://dx.doi.org/10.13039/501100000266} ``Interdisciplinary Centre for Finding, Understanding and Countering Crime in the Cloud'' grant (EP/M020320),
