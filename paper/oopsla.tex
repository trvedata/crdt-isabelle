...
This is why the axiom $\isa{broadcast{\isacharunderscore}before{\isacharunderscore}delivery}$, which mentions both the $\isa{history}$ function and the history order relation, both within the scope of the $node{\isacharunderscore}histories$ local theory, is a well-formed formula.
...
The $\isa{history}$ function is now being declared as returning a list of elements of type $\isa{event}$, declared above, rather than some abstract type variable.
