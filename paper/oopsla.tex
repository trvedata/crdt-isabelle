...
We then have a chain of equational reasoning:
\begin{align*}
...
%We write $\isa{xs}\ \isa{prefix}\ \isa{of}\ \isa{i}$ when this property holds.
%\\
%\begin{isabellebody}
...
This interpretation is achieved in Isabelle using a \isakeyword{sublocale} declaration:
\vspace{0.375em}
\begin{isabellebody}
...
The delete operation for the RGA is straightforward---it simply searches recursively for the element with a given identifier and change its flag to $\isa{True}$, which by convention we take to mean that an element has been deleted:
\vspace{0.375em}
\begin{isabellebody}
...
\ \ \ \ \ \ \ \ \ \ \ \ \ \ else\isanewline
\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ do\ {\isacharbraceleft}\ t\ {\isasymleftarrow}\ delete\ xs\ i\ {\isacharsemicolon}\ Some\ {\isacharparenleft}{\isacharparenleft}i{\isacharprime}{\isacharcomma}v{\isacharcomma}flag{\isacharparenright}{\isacharhash}t{\isacharparenright}\ {\isacharbraceright}{\isacharparenright}{\isachardoublequoteclose}%
\end{isabellebody}
\vspace{0.375em}
...
This is captured by the lemma $\isa{insert}{\isacharunderscore}\isa{no}{\isacharunderscore}\isa{failure}$:
\vspace{0.375em}
\begin{isabellebody}
...
We therefore now refine our model of the network, considering only those network executions which follow this RGA `protocol', which is made manifest as three new laws---relating broadcast and deliver events---that networks must possess.
...
The main reason to introduce $\isa{network-with-ops}$ is to serve as the `exit point' of RGA-agnostic code in our development, and the entry point of code tied to a specific RGA implementation.
...
In short, only existing elements may be deleted from the list.
\end{enumerate}
