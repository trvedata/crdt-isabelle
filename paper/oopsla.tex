...
Note that the type alias is parameterised by two type variables, ${\isacharprime}id$ and ${\isacharprime}v$, the former of which stands for some arbitrary type of identifiers, whilst the latter stands for the type of data we are actually trying to stored in the RGA.
...
As a result, in our formalisation, we found it easier to factor the definition of insertion into two functions.
The first function, $\isa{insert-body}$ never fails, and is used by the main insertion function, $\isa{insert}$, to insert an element at the head of the list.
