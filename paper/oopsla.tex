...
However, this strong consistency model may be unwarranted or unneccessary depending on the application, or it may impose an unacceptable performance degradation on the system or simply be infeasible to implement, especially in large distributed systems.
These observations motivate \emph{relaxed} consistency models, with weaker guarantees than those provided by the strong consistency model.
Relaxed consistency models are potentially better suited to application domains where changes to the shared state need not (or cannot) be propagated eagerly, and they can also provide potentially better performance characteristics than those offered by the strong consistency model.

One widely-implemented relaxed model is eventual consistency, which guarantees than when no new updates are made to the shared state all nodes will eventually agree on its contents.
...
Whilst eventual consistency may suit certain domains, the guarantees it offers clients are weak---neither system behaviour in the case where updates to shared state never cease, nor the values of intermediate reads from shared state prior to convergence, are constrained under this model, for instance.
Eventual consistency may therefore be \emph{too} relaxed, and intermediate models, lying between strong and eventual consistency, are required.
One such intermediate model is strong eventual consistency, which guarantees that whenever two nodes have received the same set of messages---possibly in a different order---their view of the shared state is always identical.
...
Unfortunately, we currently have a poor understanding of algorithms that enable strong eventual consistency in the decentralised, or peer-to-peer, setting.
This limits the use and deployment of these algorithms as peer-to-peer systems are especially attractive and widely-deployed in contexts where reliance on a central server is impractical or undesirable.
These contexts can include data centres where a central server is a bottleneck that limits scalability, but also can include laptop and mobile phone applications which have intermittent network connections, or have a local connection to a subset of network nodes, e.g. in calendar or collaborative editing applications.
This poor understanding is especially unfortunate, as it seems that peer-to-peer systems are best designed with replication algorithms which provide strong eventual consistency guarantees.
In particular, both strong and eventual consistency algorithms are inappropriate in this context as they cannot support local updates on nodes in the presence of arbitrary network partitions, and require the user to manually resolve merge conflicts, respectively.

To bolster our claim that these algorithms are poorly understood, we point out that several decentralised strong eventual consistency algorithms, published in peer-reviewed venues, that claimed to work correctly but were subsequently shown to violate their supposed guarantees.
Informal reasoning has resulted in the development of algorithms later shown incorrect, and even formal, mechanised proofs of the correctness of algorithms have later been shown to be incorrect.
