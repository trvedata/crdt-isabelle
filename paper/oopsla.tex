...
All of our CRDT implementations are ``executable'' in the sense that functioning OCaml (or Scala, SML, and Haskell) code can be obtained from our definitions using Isabelle's code extraction mechanism.
\item
...
Lastly, all three of our CRDT implementations are ``executable'' in the sense that we can use Isabelle's code extraction mechanism to obtain OCaml (or Scala, SML, and Haskell) implementations from our definitions.
We have used an extraction of one of our CRDTs---the counter---sitting above a simple network of $n$ nodes, communicating over TCP links between machines, to show that our implementations are usable in practice.
