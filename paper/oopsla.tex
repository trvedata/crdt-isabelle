...
All of our proofs are checked with the Isabelle proof assistant~\cite{DBLP:conf/tphol/WenzelPN08}, which we now briefly introduce so that the reader can easily follow our work.
...
We use one particular object logic, Isabelle/HOL---a more expressive implementation of Gordon's higher-order logic~\cite{DBLP:conf/tphol/Gordon91} than that provided by the meta-logic---as the arena for our mechanisation.
Unlike systems based on Type Theory---such as Agda~\cite{DBLP:conf/tphol/BoveDN09}, Coq~\cite{DBLP:conf/popl/HuetH14}, or Matita~\cite{DBLP:conf/cade/AspertiRCT11}---Isabelle/HOL implements a classical logic.
...
As a concrete example in Section~\ref{sect.rga} we present a mechanisation of an existing CRDT: the Replicated Growable Array (RGA).
Our mechanisation consists of two parts.
...
Combining the two parts of the mechanisation together, we see that the constrained network provides the right conditions under which all operations commute with each other.
