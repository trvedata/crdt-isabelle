...
Unlike systems based around Constructive Type Theory---such as Agda, Coq, or Matita---Isabelle/HOL implements a fully classical logic.
...
`Apply-style' tactic driven theorem proving is one method, which sees a proof obligation decomposed into simpler subgoals in a backwards-directed manner, using \emph{tactics}.
An alternative style uses the Isar structured proof language, which attempts to mimic the structure and form of human-written proofs in a completely machine-checked way, and is claimed to be closer to informal mathematical practice than the alternative `apply-style' proofs.
We use both styles in this formalisation, depending on the particular theorem that we are proving.
