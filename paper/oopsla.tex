...
As a case study in applying our framework, we formalised three operation-based CRDTs---the Replicated Growable Array, the Observed-Remove Set, and an increment-decrement Counter---and proved that they do indeed guarantee the SEC properties under all possible valid behaviours of our network model.
...
For each algorithm we then proved that its assumptions are satisfied in all possible network behaviours, and that it satisfies the preconditions of our abstract SEC specification, obtaining convergence theorems for each of the three CRDT implementations.
