...
We develop a modular and reusable framework in the Isabelle/HOL interactive theorem prover for verifying the correctness of CRDT algorithms.
We avoid correctness issues that have dogged previous mechanised proofs in this area by including a network model in our formalisation, and proving that our theorems hold in all possible executions of the network.
...
For example, this model is used in many relational databases such as PostgreSQL~\cite{postgresql}.
...
For example, version control systems such as Git or Mercurial require the user to resolve merge conflicts manually; and some ``NoSQL'' distributed database systems (for example \texttt{TODO}) adopt a \emph{last-writer-wins} policy, under which one update is chosen as the winner, and concurrent updates are discarded.
...
Large-scale deployments of SEC algorithms include datacentre-based applications using the Riak distributed database~\cite{riak}, and collaborative editing applications such as Google Docs~\cite{google-docs}.
...
In this work we use Isabelle/HOL, a generic proof assistant tool \cite{DBLP:conf/tphol/WenzelPN08}, to create a framework for reliably reasoning about the correctness of a particular class of decentralised replication algorithms.
We do this by formalising not only the replication algorithms, but also the network in which they execute, allowing us to prove that the algorithm's assumptions hold in all possible executions of the network.
...
These mechanised proofs demonstrate that our framework is highly reusable: we developed proofs of the set and counter CRDTs in a few hours and with little CRDT-specific code, by using a fixed pattern that applies to all of our CRDT proofs.
\item
...
We now provide a brief overview of the structure of the rest of the paper.
In Section~\ref{sect.high-level.proof.strategy} we provide a high-level English-language description of our proof strategy, and in Section~\ref{subsect.an.overview.of.isabelle} we briefly describe the Isabelle/HOL interactive proof assistant which we used to mechanise our proof, providing enough material for the unfamiliar reader to be able to follow the rest of our paper.
In Section~\ref{sect.abstract.convergence} we describe the mechanised proof of abstract convergence theorem, which we claim is the essence of why strong eventual consistency algorithms converge, whilst in Section~\ref{sect.network} we describe an axiomatic model of asynchronous unreliable causal broadcast networks, and related definitions---a standard abstraction in distributed systems theory.
In Section~\ref{sect.rga} we describe a mechanisation and formal proof of correctness of the Replicated Growable Array (RGA) replicated ordered list data type, and in Section~\ref{sect.simple.crdts} we describe the mechanisation and proof of correctness of two more CRDTs.
In Section~\ref{sect.relatedwork} we survey related work, and finally in Section~\ref{sect.conclusion} we conclude.

All of our Isabelle/HOL definitions and proofs have been submitted as anonymised supplementary material for the reviewing process.
