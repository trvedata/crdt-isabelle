...
\end{figure}
>>>>>>> e0b66fcfbf86cf750010cb9b2abaef028781f5d9
...
that is $\isa{m1} \prec \isa{m2}$, if both messages has been broadcasted by the
...
The correctness of operation-based CRDTs relies on \emph{causal networks}.
Since this is a stronger requirement, we separate this axiom into a new
\textbf{locale}, making the whole implementation modular and adaptable to new
formalisations that do not depend on causality.  We say that the network
satisfy the causality property whenever a message \isa{m1} happens before
\isa{m2} and a node \isa{j} has received the message \isa{m2}, then the node
...
Interpreting a local theory—such as hoo above—involves providing a concrete implementation of f coupled with a proof that the concrete implementation satis es the associated law.

It can be shown that this definition of \emph{happens-before} forms a
preorder, matching the requirement of the local theory of the previous section.
The two local theories --the network and the happens-before locale from the previous section-- can now be connected in Isabelle by a sublocale statement. It is enough to prove that the network satisfy every axiom (that is, that the We prove that the 

Once interpreted, all functions, de nitions, and theorems made within the hoo local theory become available to use for that concrete implementation.
